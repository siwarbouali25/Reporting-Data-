...
Table purpose:
  ...
  This table is a structured qualitative evidence table. It is NOT the final
  IFRS S2 report section. It captures evidence that a reporting agent can later
  use to generate the final transition plan disclosure.
...
def build_skeleton(
    ...
    for _, ent in entities.iterrows():
        ...
        for yr in years:
            ...
            # -------------------------------------------------
            # KPI block: entity-level climate / GHG / energy targets
            # -------------------------------------------------
            kpi_mask = (
                year_filter(kpi, yr)
            ...
            # -------------------------------------------------
            # Scenario block: entity-level if possible, group fallback otherwise
            # -------------------------------------------------
            sc_yr = sc[
...
- Avoid vague phrases such as "committed to a sustainable future", "leading the transition", or "striving to".
- Use precise wording such as "the evidence indicates", "the source data records", "the plan is supported by".
...
def build_user_prompt(row: dict) -> str:
    ...
    carbon_price_str = (
        f"EUR {row['carbon_price_eur_per_tco2e']:.2f}/tCO2e; "
...
def call_openai(
    ...
    for attempt in range(retries):
        ...
        try:
            ...
            tracker.log(
                row_id, entity_id, reporting_year,
                0, 0, 0,
